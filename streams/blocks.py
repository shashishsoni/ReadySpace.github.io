...
class Cardsblocks(blocks.StructBlock):
...
class ImageAndTextBlocks(blocks.StructBlock):
    ...
    text = blocks.TextBlock(max_length=1200, required=False, help_text = 'max lenth of 500 character')
    link = Link()

    class Meta:
        template = "streams/image_and_text_blocks.html"
        icon = "image"
        label = "Image & Text"
...
class CallToActionBlock(blocks.StructBlock):
    ...
    title = blocks.CharBlock(max_length=750, help_text='Max length of 200 characters.')
    link = Link()
